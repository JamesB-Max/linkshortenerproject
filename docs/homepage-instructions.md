...
| Decision | Rationale |
|---|---|
| **Only `card` added** | It's the only Shadcn component not yet installed that's genuinely needed. `button` is already present; no other primitives (badge, separator, etc.) are required for this layout. |
| **`features` array defined at module scope** | Keeps the JSX clean and avoids cluttering the `async` component body with static data. Server components have no issue with module-level constants. |
| **`mode="modal"` on every `SignUpButton` / `SignInButton`** | Hard requirement from `clerk-routing-modals.md` — no exceptions, not even for the bottom CTA. |
| **`flex flex-1 flex-col` on `<main>`** | Matches the layout's `body` class (`flex flex-col`) so the hero section stretches to fill remaining viewport height on short pages. |
| **`bg-muted/40` on the Features section** | Uses a CSS variable–driven Tailwind colour, fully compatible with the dark/light theme toggle already wired in `layout.tsx`. |
| **No `<CardFooter>` used** | Not needed — cards only contain a header and body content. Including it would add unnecessary DOM nodes. |
